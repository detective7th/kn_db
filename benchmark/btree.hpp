...
namespace trees {
    ...
    template <class Key, class Value> BTree<Key, Value>::Node::Node() {
      is_leaf = false;
      vals_size = 0;
      childs_size = 0;
      id = 0;
    ...
    template <class Key, class Value> BTree<Key, Value>::Node::Node(size_t cap) {
      is_leaf = false;
    ...
    void BTree<Key, Value>::Node::insertValue(Key key, Value val) {
      ...
      auto lb_iter = std::lower_bound(
          this->vals.data(), this->vals.data() + vals_size, kv,
    ...
    void BTree<Key, Value>::Node::insertChild(Key key, typename Node::Ptr C) {
      ...
      auto lb_iter = std::lower_bound(
          this->vals.data(), vals.data() + vals_size, kv,
          [](const std::pair<Key, Value> &l, const std::pair<Key, Value> &r) {
            return l.first < r.first;
          });
      if (lb_iter != this->vals.data() + vals_size) {
    ...
    template <class Key, class Value> BTree<Key, Value>::BTree(size_t N) : n(N) {
    ...
    template <class Key, class Value> BTree<Key, Value>::~BTree() {
      m_root = nullptr;
      this->cache->clear();
      delete this->cache;
    }
    ...
    template <class Key, class Value>
    typename BTree<Key, Value>::Node::Ptr BTree<Key, Value>::getNode(
        const typename BTree<Key, Value>::Node::Weak &w) const {
      return &cache->at(w);
    }
    
    template <class Key, class Value>
    typename BTree<Key, Value>::Node::Ptr
    BTree<Key, Value>::getNode(const typename BTree<Key, Value>::Node::Weak &w) {
      return &cache->at(w);
    }
    ...
    template <class Key, class Value> Value BTree<Key, Value>::find(Key key) const {
    ...
    template <class Key, class Value>
    bool BTree<Key, Value>::iner_find(Key key, typename Node::Ptr cur_node,
                                      typename Node::Ptr &out_ptr,
                                      Value &out_res) const {
      if (cur_node->is_leaf) {
        auto find_res = std::lower_bound(
            cur_node->vals.data(), cur_node->vals.data() + cur_node->vals_size,
      ...
      auto low_bound = std::lower_bound(
          cur_node->vals.data(), cur_node->vals.data() + cur_node->vals_size, kv,
    ...
    template <class Key, class Value>
    bool BTree<Key, Value>::isFull(
        const typename BTree<Key, Value>::Node::Ptr node) const {
    ...
    template <class Key, class Value>
    void BTree<Key, Value>::to_stream(
        std::ostream &stream,
        const typename BTree<Key, Value>::Node::Ptr &root) const {
      stream << "{  [";
